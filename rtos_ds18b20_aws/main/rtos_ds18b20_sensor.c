...
#define TEMP_READ_INTERVAL_MS 10000  // Read temperature every 10 seconds (increased from 2s)
...
static void timer_callback(void* arg) {
    ...
    if (reading.temperature_celsius > -999.0) {
        ...
        xQueueSend(tempQueue, &reading, 0);
        
        // Also send to AWS IoT publishing queue (same queue used in this integration)
        // The MQTT task will pick this up for publishing
    }
}
...
void storeTemperatureTask(void *pvParameters) {
    ...
    while (1) {
        if (xQueueReceive(tempQueue, &receivedReading, portMAX_DELAY)) {
            ...
            // Push the temperature reading back into the queue for AWS IoT
            // This ensures both the local storage task and AWS IoT task can process it
            xQueueSendToBack(tempQueue, &receivedReading, 0);
        }
...
void start_temperature_sensor(void) {
    ESP_LOGI(TAG, "Starting DS18B20 Temperature Sensor...");
    
    // Initialize DS18B20
    ds18b20_init();
    
    // Create queue if it hasn't been created yet
    if (tempQueue == NULL) {
    ...
    // Create storage task if it's needed as a standalone task
    // Note: In our integration, this will be called from app_main.c
} 